...
class OneFlowDiffusionPipeline(ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        # extract them here
        expected_modules = expected_modules = set(inspect.signature(pipeline_class.__init__).parameters.keys()) - set(["self"])
        ...
        if len(missing_modules) > 0 and missing_modules <= set(passed_class_obj.keys()):
            ...
            raise ValueError(
                f"Pipeline {pipeline_class} expected {expected_modules}, but only {passed_modules} were passed. {missing_modules} missing."
            )
