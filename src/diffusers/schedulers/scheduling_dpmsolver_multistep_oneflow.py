    ...
    def convert_model_output(
        ...
        if self.config.algorithm_type == "dpmsolver++":
            ...
            if self.config.thresholding:
                ...
                #)
                dynamic_max_val = torch.from_numpy(np.quantile(
                    torch.abs(x0_pred).reshape((x0_pred.shape[0], -1)).cpu().numpy(), self.config.dynamic_thresholding_ratio, dim=1
                )).to(x0_pred.dtype).to(x0_pred.device)
