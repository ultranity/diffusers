...
class OneFlowUNet2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        #print(self.config, self._internal_dict) #oneflow overrides .config
        if self._internal_dict.center_input_sample:
            sample = 2 * sample - 1.0
        ...
        t_emb = t_emb.to(dtype=torch.float16)#.to(dtype=self.dtype)
